...
{
    namespace Shop {
        public class DiamondItemScrollView : MonoBehaviour
        {
            ...
            private Dictionary<int, DiamondShopModel> diamondItemTable = new Dictionary<int, DiamondShopModel>();
            ...
            public void Init()
            {
                diamondItemTable = ShopDataManager.instance.GetDiamondShopItemTable();

                if (diamondItemTable == null || diamondItemTable.Count == 0)
                {
                    //Debug.LogWarning("Diamon shop item table is null or There are no items!!");
                    return;
                }

                for(int i = 0; i < diamondItemTable.Count; ++i)
                { 
                    MakeSubItem();
                }

                int pageCount = diamondItemTable.Count / PAGEMAXITEMCOUNT;
                int offset = diamondItemTable.Count % PAGEMAXITEMCOUNT;
                if (offset > 4)
                    offset = 0;
                ...
                // 스크롤뷰의 넓이를 계산한다.
                float totalScrollViewWidth = DEFAULTSCROLLVIEWWIDTH + SCROLLVIEWWIDTHPERITEM * (diamondItemTable.Count - 1);
            ...
            {
                Managers.UI.MakeSubItemAsync<DiamondShopItem>(gridTransform, null,
                    (GameObject itemGO) =>
                    {
                        itemGO.transform.localScale = Vector3.one;
                        itemGO.transform.localPosition = Vector3.zero;
                        items.Add(itemGO);
                        if (items.Count < diamondItemTable.Count)
                            return;

                        int idx = 0;
                        foreach (DiamondShopModel param in diamondItemTable.Values)
                        {
                            items[idx].GetComponent<DiamondShopItem>().Init(param.ID, param.objectName, param.priceType, param.priceCount,
            ...
            {
                if (diamondItemTable.Count > PAGEMAXITEMCOUNT)
                {
