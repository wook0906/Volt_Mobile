...
public class ExcelImporterMaker : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        {
            Debug.Log($"Path:{Path.Combine(Application.dataPath, "Volt_Json_key.txt")}");
            using (FileStream stream = File.Open(Path.Combine(Application.dataPath, "Volt_Json_key.txt"), FileMode.Open, FileAccess.Read))
            {
                byte[] array = new byte[stream.Length];
                stream.Read(array, 0, array.Length);
                fileKey = Encoding.UTF8.GetString(array);
            }
            EditorPrefs.SetString(s_key_prefix + fileName + ".className", className);
			Debug.Log(s_key_prefix + fileName + ".className");
            ExportUIData();
            ExportAtlasPathImporter();
            ExportDecryptScript();
    ...
    void ExportUIData()
    {
        ...
        foreach (ExcelRowParameter row in typeList)
        {
            if(row.isEnable)
            {
                ...
                {
                    builder2.AppendLine();
                    builder2.AppendFormat("          clone.{0} = this.{1};", row.name, row.name);
                }
            }
        }
        builder2.AppendLine();
        builder2.Append("           return clone;");
        ...
        entittyTemplate = entittyTemplate.Replace("$Clones$", builder2.ToString());
    ...
    void ExportUIImporter()
    {
        Debug.Log("ExportImporter");
        ...
        importerTemplate = importerTemplate.Replace("$EXPORT_PATH$", Path.ChangeExtension(filePath, ".json"));
        ...
        importerTemplate = importerTemplate.Replace("$KEY$", fileKey);
        importerTemplate = importerTemplate.Replace("$ID_TYPE_FILE_PATH$", "Assets/Editor/ACHIDTypeTemplate.txt");
        importerTemplate = importerTemplate.Replace("$DATA_TYPE$", fileName);
    ...
    void ExportAtlasPathImporter()
    {
        Debug.Log("ExportAtlasImporter");
        string templateFilePath = (sepalateSheet) ? "Assets/Editor/ExportTemplate2.txt" : "Assets/Editor/ExportTemplate3.txt";
    ...
    void ExportDecryptScript()
    {
        string templateFilePath = "Assets/Editor/DecryptScriptTemplate.txt";
        string scriptTemplate = File.ReadAllText(templateFilePath);

        scriptTemplate = scriptTemplate.Replace("$KEY$", fileKey);
        scriptTemplate = scriptTemplate.Replace("$IMPORT_PATH$", "Assets\\\\Resources\\\\Data\\\\" + fileName + ".json");
        scriptTemplate = scriptTemplate.Replace("$ExcelData$", className + "_Decrypt");
        scriptTemplate = scriptTemplate.Replace("$ClassName$", className);

        Directory.CreateDirectory("Assets/Classes/Decrypt/");

        string FilePath = "Assets/Classes/Decrypt/" + className + "_Decrypt.cs";
        if(File.Exists(FilePath))
        {
            File.Delete(FilePath);
        }
